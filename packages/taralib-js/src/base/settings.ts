...
// New code should use SettingsHandler class directly.

let defaultHandler: SettingsHandler | null = null;

function getDefaultHandler(): SettingsHandler {
    if (!defaultHandler) {
        defaultHandler = new SettingsHandler();
    }
 ...
 *
 * @param workingDir - Optional working directory. Defaults to process.cwd()
 * @deprecated Use SettingsHandler class instead
 */
export function refreshSettings(workingDir?: string): void {
    getDefaultHandler().refresh(workingDir);
}
 ...
 * @deprecated Use SettingsHandler class instead
 */
export function getRawValue(key: string, source: SettingSource): any {
    return getDefaultHandler().getRawValue(key, source);
}
...
export function getSetting(key: string, defaultValue?: any): any {
    return getDefaultHandler().getSetting(key, defaultValue);
}
...
export function isSettingsLoaded(): boolean {
    return getDefaultHandler().isLoaded();
}

/**
 * Get the default settings handler instance (for internal use).
 * Creates one if it doesn't exist.
 * @internal
 */
export function getDefaultSettingsHandler(): SettingsHandler {
